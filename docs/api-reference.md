# API reference (preserved from the upstream README)

These endpoint notes and example responses come from the original
[ahivert/tgtg-python](https://github.com/ahivert/tgtg-python) README. They are kept here
because the response shapes are hard-won reference material and this checkout has no git
history to recover them from. Field names drift as TooGoodToGo ships new API versions, so
treat the examples as a guide rather than a contract.
